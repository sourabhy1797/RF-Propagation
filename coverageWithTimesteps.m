function varargout = coverageWithTimesteps(txs, varargin)
...
disp("validate all sites are all geogrphic")
tic
...
disp("Get Site Viewer and validate web graphics")
tic
...
disp("trim grid locaitons to those which are not inside buildings")
tic
...
disp("Use try/catch in case cancel error thrown from waitbar dialog")
tic
try
    if strcmp(rxLocationsLayout,'grid')
        ...
        disp("Compute signal strength at each rxsite in the grid")
        tic
...
disp("Merge data of rx sites for use in single image, where signal strength")
disp("at an rx site is the max strength due to any tx.")
tic
if numTx > 1
    data = max(data, [], 1);
end
...
disp("Set the IDs of the appropriate Site Graphics")
tic
for i = 1:numel(txs)
...
disp("Hide progress dialog")
tic
viewer.hideProgressDialog;
toc

% Cancel forced exit cleanup, since normal execution has completed
disp("Cancel forced exit cleanup, since normal execution has completed")
tic
...
disp("Function Call: validateSignalStrength")
tic
try
...
disp("Function Call: validateMaxRange")
tic
try
...
disp("Function Call: isWithinAnyBuilding")
tic
% Check if locations are within footdisp of any buildings
